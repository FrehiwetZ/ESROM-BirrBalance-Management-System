...
export default function EmployeePortal() {
  ...
  useEffect(() => {
    if (user && user.role === "employee" && !activeCafe) {
      setCafesLoading(true);
      apiGet("/api/employee/cafes")
        .then((res) => {
          setCafesList(res.data);
        })
        .catch((e) => console.error("Failed to load cafes", e))
  ...
  if (!activeCafe) {
    return (
      <div className="flex-1 max-w-xl mx-auto w-full px-4 py-10 space-y-4">
        <h2 className="text-xl font-semibold">Choose a cafe</h2>
        {cafesLoading && <p className="text-gray-500">Loading cafes…</p>}
        {!cafesLoading && cafesList.length === 0 && (
          <p className="text-gray-500">No cafes available yet.</p>
        )}
        {cafesList.map((cafe: any) => (
          <button
            key={cafe.id}
  ...
  return (
    <div className="flex-1 max-w-7xl mx-auto w-full px-4 py-6 md:py-10 space-y-8 font-sans relative">
      ...
      {activeTab === "notifications" && <EmployeeNotificationsPage />}

      {/* Floating Toast Notification overlay */}
      {toast && (
        <div
          id="portal-toast"
          className="fixed bottom-6 right-6 z-55 max-w-md animate-slideIn"
        >
          <div
            className={`p-4 rounded-2xl shadow-2xl border text-xs font-bold flex items-start gap-3 ${
              toast.type === "success"
...
function EmployeeDashboard({
  ...
  const downloadPDF = () => {
    ...
    pdf.text("Present this card at the cafeteria counter", 52.5, 125, {
      align: "center",
    });
    ...
    <div className="space-y-8 animate-fadeIn">
      ...
      {isLowBalance && (
        <div
          ...
          className="p-4 rounded-2xl bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/60 text-warning text-xs font-bold flex gap-3 items-start animate-fadeIn shadow-sm"
        >
          <AlertTriangle className="w-5 h-5 flex-shrink-0 mt-0.5 animate-bounce" />
          <div className="space-y-1">
            <h4 className="font-black uppercase tracking-wider text-[11px]">
              Low Balance Alert
            </h4>
            <p className="font-medium text-slate-700 dark:text-slate-300">
              Your balance is running low. {formatETB(user.balance)} remaining
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-3xl p-5 border border-slate-100 shadow-sm flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[11px] text-subtle-text font-medium mt-0.5">
                  You dined 5 times in the last 7 days! Keep it up!
                </p>
              </div>
            </div>
        ...
        <div className="bg-white dark:bg-[#0A1628] rounded-3xl p-6 border border-border-default shadow-sm dark:shadow-[0_0_15px_rgba(59,130,246,0.15)] dark:border-brand-primary/30 flex flex-col items-center justify-between space-y-4">
          <div className="text-center">
            <h3 className="text-sm font-black text-text-primary dark:text-white tracking-tight">
              Your Cafeteria QR Code
            </h3>
            <p className="text-[10px] text-text-subtle dark:text-slate-400 font-medium mt-0.5">
              Show this to the waiter to pay instantly
            </p>
          </div>
          ...
          <div className="pt-3 border-t border-slate-100 dark:border-slate-800 text-center w-full mt-2">
            ...
            <p className="text-[10px] text-slate-400 dark:text-slate-500 font-medium mt-0.5">
              This is your permanent cafeteria QR code. Keep it secure and do
              not share it.
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm lg:col-span-1 flex flex-col justify-between space-y-4">
          <div>
            ...
            <p className="text-[10px] text-subtle-text font-medium mt-0.5">
              Chef's daily lunch recommendation
            </p>
          </div>
          ...
          <div className="space-y-1 text-xs">
            ...
            <p className="text-[11px] text-subtle-text leading-snug">
              Spicy chicken drumsticks cooked in berbere red sauce with
              hard-boiled eggs and injera bread.
            </p>
          </div>
        ...
        <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm lg:col-span-2 space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-sm font-black text-primary tracking-tight">
                Recent Dining Activities
              </h3>
              <p className="text-[10px] text-subtle-text font-medium mt-0.5">
                Review your latest meal Birr redemptions
              </p>
            </div>
            <button
              onClick={() =>
                navigate("/employee/orders", {
                  state: { defaultTab: "order-history" },
                })
              }
          ...
          <div className="space-y-3">
            {myOrders.slice(0, 3).map((item) => (
              <div
                key={item.id}
                className="p-3 hover:bg-slate-50 rounded-2xl border border-slate-50 flex justify-between items-center transition-all"
              >
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-primary max-w-xs truncate">
                      {item.order_items
                        .map(
                          (i: any) =>
                            `${i.item_name_snapshot} (x${i.quantity})`,
                        )
                        .join(", ")}
                    </h4>
                    <span className="text-[10px] text-slate-400">
                      {new Date(item.created_at).toLocaleDateString()} &bull;
                      Main Cafeteria
                    </span>
                  </div>
                </div>
  ...
  const handleCheckout = async () => {
    ...
    if (total > user.balance) {
      setToast({
        type: "error",
        message:
          "Overdraft! You don't have enough ETB to pay for this meal tray.",
      });
      return;
    }
    ...
    try {
      ...
      setToast({
        type: "success",
        message: `${formatETB(res.data.total_amount)} deducted. Remaining: ${formatETB(res.data.remaining_balance)}`,
      });

      if (res.data.remaining_balance < monthlyAllowance * 0.2) {
        setTimeout(() => {
          setToast({
            type: "warning",
            message: `Your balance is running low. ${formatETB(res.data.remaining_balance)} remaining`,
          });
        }, 3000);
      }
    } catch (e: any) {
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-5">
        <div>
          ...
          <p className="text-xs text-subtle-text font-medium uppercase tracking-wider mt-1">
            Browse active food selections, fill your tray and check out
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            value={activeCafe.id}
            onChange={(e) => {
              const selected = cafesList.find(
                (c: any) => c.id === Number(e.target.value),
              );
              if (selected) setActiveCafe(selected);
            }}
        ...
        <div className="bg-white rounded-3xl p-6 md:p-8 max-w-lg mx-auto shadow-xl border border-slate-100 space-y-6 text-center text-xs">
          <div className="flex flex-col items-center space-y-2">
            ...
            <p className="text-subtle-text">
              Your lunch ticket has been submitted to the kitchen.
            </p>
          </div>

          <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl text-left space-y-2">
            <p className="text-[10px] font-bold text-subtle-text uppercase">
              Order Receipt
            </p>
          ...
          {feedbackSent ? (
            <p className="p-3 bg-green-50 rounded-xl border border-green-100 text-success font-bold text-[11px]">
              Thank you for your rating! Feedback submitted to the cafeteria
              management.
            </p>
          ) : (
            <form
            ...
            >
              <div>
                ...
                <p className="text-[10px] text-slate-400 mt-0.5">
                  Help us maintain quality at the corporate cafeteria
                </p>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-2 xl:grid-cols-4 gap-3 md:gap-6">
              {filteredItems.map((item) => (
                <div
                  key={item.id}
                  className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden flex flex-col justify-between group hover:shadow-md transition-all"
                >
                  ...
                  <div className="p-4 space-y-3 flex-1 flex flex-col justify-between">
                    <div className="space-y-1">
                      <div className="flex justify-between items-start gap-1">
                        ...
                        <span className="text-[11px] font-black text-secondary flex-shrink-0">
                          {formatETB(item.price)}
                        </span>
                      </div>
                      <p className="text-[10px] text-subtle-text line-clamp-2 leading-relaxed">
                        {item.description}
                      </p>
                    </div>
                    <button
          ...
          <div className="lg:col-span-4 bg-white rounded-3xl p-5 border border-slate-100 shadow-sm space-y-5">
            <div>
              ...
              <p className="text-[10px] text-subtle-text font-medium mt-0.5">
                Items selected for lunch ticket validation
              </p>
            </div>
            ...
            {cart.length === 0 ? (
              <div className="py-12 text-center text-slate-300 flex flex-col items-center justify-center space-y-2">
                ...
                <p className="text-xs text-subtle-text font-medium">
              ...
              <div className="space-y-4">
                  ...
                  {cart.map((c) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-extrabold text-primary w-5 text-center">
                          {c.quantity}
                        </span>
                        <button
                ...
                {isLowBalance && (
                  <div
                    id="low-balance-order-alert"
                    className="p-3 bg-amber-50 dark:bg-amber-950/20 rounded-xl border border-amber-100 dark:border-amber-900/40 text-warning text-[10px] font-bold flex gap-2 items-start"
                  >
                    <AlertTriangle className="w-4 h-4 flex-shrink-0 mt-0.5" />
                    <p>
                      Your balance is running low. {formatETB(user.balance)}{" "}
                      remaining.
                    </p>
                  </div>
                )}
                ...
                {getCartTotal() > user.balance && (
                  <div className="p-3 bg-red-50 rounded-xl border border-red-100 text-danger text-[10px] font-bold flex gap-2 items-start animate-shake">
                    <AlertTriangle className="w-4 h-4 flex-shrink-0 mt-0.5" />
                    <p>
                      Overdraft detected. Please remove some items from your
                      tray to proceed.
                    </p>
                  </div>
                )}
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      <div>
        ...
        <p className="text-xs text-subtle-text font-medium uppercase tracking-wider mt-1">
          Audit log records of your cafeteria redemptions
        </p>
      </div>
                ...
                return (
                  <React.Fragment key={item.id}>
                    <tr className="hover:bg-slate-50/40 transition-colors">
                      ...
                      <td
                        className="py-3 px-4 font-semibold text-primary max-w-sm truncate hidden md:table-cell"
                        title={item.items
                      ...
                      <td className="py-3 px-4 text-right font-extrabold text-danger">
                    ...
                    {isExpanded && (
                      <tr className="bg-slate-50/50 md:hidden">
                        <td
                          colSpan={5}
                          className="py-3 px-6 text-[11px] text-slate-600 space-y-1"
                        >
                          <p>
                            <strong>Items:</strong>{" "}
                            {item.items
  ...
  return (
    <div className="space-y-6 animate-fadeIn text-xs">
      ...
      <div>
        ...
        <p className="text-xs text-subtle-text font-medium uppercase tracking-wider mt-1">
          Manage passwords and review subsidy allocation records
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm space-y-5">
            <div>
              ...
              <p className="text-[10px] text-subtle-text mt-0.5">
                Verify old passwords before setting a new login key
              </p>
            </div>

            {passError && (
              <p className="p-3 bg-red-50 rounded-xl border border-red-200 text-danger font-bold">
                {passError}
              </p>
            )}

            {passSuccess && (
              <p className="p-3 bg-green-50 rounded-xl border border-green-200 text-success font-bold">
                {passSuccess}
              </p>
            )}
        ...
        <div className="lg:col-span-7 bg-white rounded-3xl p-6 border border-slate-100 shadow-sm space-y-6">
          <div>
            <h3 className="text-sm font-black text-primary tracking-tight">
              Monthly Birr Allowance History
            </h3>
            <p className="text-[10px] text-subtle-text font-medium mt-0.5">
              Corporate credits allocated to your account by month
            </p>
          </div>
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      {subTab === "order-now" ? (
        ...
        <EmployeeOrdersHistory myOrders={myOrders} />
      )}
    </div>
  );
}
...
function EmployeeTransactionsPage({
  ...
  const allTransactions = React.useMemo(() => {
    ...
    const allocation =
      user.monthlyAllocation || (user.balanceTier === "Tier 1" ? 5000 : 3000);
    ...
    if (allocationHistory && allocationHistory.length > 0) {
      allocationHistory.forEach((alloc) => {
        const monthStr = new Date(alloc.date).toLocaleString("default", {
          month: "long",
        });
          ...
          list.push({
            ...
            orderDetails: {
              ...
              deliveryConfirmationTime:
                order.deliveryConfirmationTime ||
  ...
  const filteredTransactions = React.useMemo(() => {
    return allTransactions.filter((t) => {
      ...
      if (searchQuery) {
        ...
        const matchOrderId =
          t.orderId.toLowerCase().includes(q) || t.id.toLowerCase().includes(q);
        const matchFood =
          t.orderDetails?.items.some((item) =>
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute(
      "download",
      `esrom_transactions_export_${new Date().toISOString().split("T")[0]}.csv`,
    );
  ...
  const handleExportXLSX = () => {
    ...
    link.setAttribute(
      "download",
      `esrom_transactions_export_${new Date().toISOString().split("T")[0]}.xlsx`,
    );
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  ...
  return (
    <div className="space-y-8 animate-fadeIn transaction-history-page p-6 rounded-3xl">
      ...
      <div>
        ...
        <p className="text-sm text-subtle-text font-medium mt-1">
          A full record of all your balance movements
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="summary-card credited bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 flex items-center justify-between">
          <div className="space-y-1">
            <p className="label text-subtle-text dark:text-slate-400">
              Total Credited This Month
            </p>
            <p className="amount text-success dark:text-success">
              {formatETB(juneStats.totalCredited)}
            </p>
          </div>
        ...
        <div className="summary-card deducted bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 flex items-center justify-between">
          <div className="space-y-1">
            ...
            <p className="amount text-danger dark:text-danger">
              {formatETB(juneStats.totalDeducted)}
            </p>
          </div>
        ...
        <div className="summary-card remaining bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 flex items-center justify-between">
          <div className="space-y-1">
            ...
            <p className="amount text-primary dark:text-white">
              {formatETB(user.balance)}
            </p>
          </div>
                  ...
                  {paginatedTransactions.map((t, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-4 px-4 font-semibold text-primary dark:text-slate-200">
                        ...
                        <td
                          className={`py-4 px-4 text-right font-black text-sm ${
                            t.type === "Credit"
                              ? "amount-credit text-success"
                              : t.type === "Expiry"
                                ? "text-warning"
                                : "amount-deduction text-danger"
                          }`}
                        >
                          {t.type === "Credit" ? "+" : "−"}
                          {formatETB(t.amount)}
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800 grid grid-cols-2 gap-4 text-xs">
                <div>
                  ...
                  <span className="font-extrabold text-primary dark:text-slate-200">
                    {selectedOrder.cafe}
                  </span>
                </div>
                <div>
                ...
                <div>
                  ...
                  <span className="font-extrabold text-primary dark:text-slate-200">
                    {selectedOrder.orderDetails?.waiterName}
                  </span>
                </div>
                <div>
                  <span className="text-slate-400 font-bold block">
                  ...
                  <span className="font-extrabold text-primary dark:text-slate-200">
                    {selectedOrder.orderDetails?.deliveryConfirmationTime}
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-3 pt-2">
                <h3 className="text-xs font-black text-primary dark:text-white uppercase tracking-wider">
                  Redeemed Items
                </h3>
                <div className="divide-y divide-slate-100 dark:divide-slate-800 max-h-64 overflow-y-auto pr-1 no-scrollbar">
                  {selectedOrder.orderDetails?.items.map((item, index) => (
                    ...
                    >
                      <div>
                        <p className="font-extrabold text-primary dark:text-white">
                          {item.name}
                        </p>
                        <span className="text-[10px] text-slate-400 font-medium">
                          Quantity: {item.quantity} × {formatETB(item.price)}
                        </span>
                      </div>
                      <span className="font-bold text-slate-700 dark:text-slate-300">
                        {formatETB(item.quantity * item.price)}
                      </span>
                    </div>
                  ))}
                </div>
            ...
            <div className="border-t border-slate-100 dark:border-slate-800 pt-5 space-y-4">
              <div className="flex justify-between items-center text-xs">
                ...
                <span className="text-base font-black text-danger">
                  -{formatETB(selectedOrder.amount)}
                </span>
              </div>
              <div className="flex justify-between items-center text-xs">
                <span className="text-slate-500 font-bold">
                ...
                <span className="text-base font-black text-primary dark:text-accent-blue">
                  {formatETB(selectedOrder.balanceAfter)}
                </span>
              </div>
              <button
                onClick={() => setSelectedOrder(null)}
