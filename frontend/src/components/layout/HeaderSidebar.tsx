...
export function EsromLogo({
  className = "h-8",
  light = false,
  collapsedResponsive = false,
}: {
  className?: string;
  light?: boolean;
  collapsedResponsive?: boolean;
}) {
  return (
    <div className={`flex items-center gap-2.5 ${className}`}>
      ...
      <div
        className={`flex flex-col ${collapsedResponsive ? "xl:flex md:hidden" : "flex"}`}
      >
...
export function Sidebar({
    ...
    <div className="flex flex-col h-full bg-sidebar text-text-secondary overflow-hidden">
        ...
        {links.map((link) => {
          ...
          return (
            ...
            >
              ...
              <Icon
                className={`w-5 h-5 transition-colors ${isActive ? "text-text-sidebar-active" : "text-slate-500 dark:text-slate-600"}`}
              />
              <span className="xl:block md:hidden block truncate">
                {link.label}
              </span>
      ...
      <div className="p-4 border-t border-border-default">
        ...
        {(user.role === "employee" ||
          ...
          user.role === "cafe") && (
          <div
            className="theme-toggle-wrapper mb-3 xl:flex md:hidden flex"
            onClick={toggleTheme}
          >
  ...
  return (
    <>
      ...
      <aside
        id="main-sidebar"
        className="hidden md:flex flex-col xl:w-[240px] md:w-16 bg-sidebar text-text-secondary border-r border-border-default h-screen sticky top-0 transition-all duration-300 z-30 flex-shrink-0"
      >
        {renderContent(false)}
      </aside>
      ...
      <div
        className={`fixed inset-0 z-[9999] md:hidden transition-opacity duration-300 ${mobileOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
      >
        <div
          onClick={onClose}
        ...
        ></div>
        <aside
          className={`sidebar fixed top-0 bottom-0 left-0 flex flex-col w-[80vw] max-w-[300px] bg-sidebar text-text-secondary h-full shadow-2xl z-[10000] ${mobileOpen ? "open" : ""}`}
        >
          {renderContent(true)}
        </aside>
      </div>
...
export function TopNav({ onMenuClick }: { onMenuClick?: () => void }) {
  ...
  return (
    <>
      <header
        id="top-navbar"
        className="bg-white dark:bg-primary border-b border-slate-200 dark:border-slate-800 sticky top-0 z-40 px-4 py-3 xl:px-8 shadow-sm h-16 flex items-center relative"
      >
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div className="relative">
              ...
              {showNotificationsMenu && (
                <div
                  ...
                  className="absolute right-0 mt-3 w-80 bg-white dark:bg-primary border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl z-50 overflow-hidden"
                >
                      ...
                      notifications.slice(0, 5).map((notif) => (
                        ...
                        >
                          ...
                          <div className="flex-1">
                            <h4 className="text-xs font-bold text-dark-text dark:text-white">
                              {notif.title}
                            </h4>
                            <p className="text-[11px] text-subtle-text dark:text-slate-300 mt-0.5 leading-tight">
                              {notif.message}
                            </p>
            ...
            <div className="relative">
              ...
              {showProfileMenu && (
                <div
                  id="profile-dropdown"
                  className="absolute right-0 mt-3 w-56 bg-white dark:bg-primary border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl z-50 overflow-hidden"
                >
